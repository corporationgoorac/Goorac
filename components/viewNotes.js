...
class ViewNotes extends HTMLElement {
    ...
    render() {
        ...
        <style>
            ...
            .vn-meta-row {
                ...
                font-size: 0.8rem; color: #8e8e93; font-weight: 500;
            }
            ...
            .vn-bubble-text { 
                ...
                background: #2c2c2e; padding: 18px 26px; border-radius: 26px;
            ...
            /* MY NOTE BUTTONS */
            .vn-my-actions { display: flex; flex-direction: column; gap: 10px; margin-top: 10px; }
            .vn-btn {
            ...
            .vn-btn-primary { background: #333; color: white; }
            .vn-btn-danger { background: rgba(255, 59, 48, 0.1); color: #ff453a; }
    ...
    async open(noteData, isOwnNote = false) {
        ...
        // 1. Initial Render (Quick)
        this.updateContent(noteData, isOwnNote, null); // Pass null for userData initially
        ...
        // 4. Fetch Fresh User Data (Verification, Real Name, etc.)
        try {
            const userDoc = await this.db.collection('users').doc(noteData.uid).get();
            if (userDoc.exists) {
                // Re-render with fresh user data
    ...
    updateContent(note, isOwn, userData) {
        ...
        // Use fresh data if available, else fallback to note data
